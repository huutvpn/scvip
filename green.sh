...
rm -rf green.sh >/dev/null 2>&1
apt install git curl -y >/dev/null 2>&1
apt install python -y >/dev/null 2>&1

CEKEXPIRED() {
    today=$(date -d "+1 day" "+%Y-%m-%d")
    Exp1=$(curl -sS https://raw.githubusercontent.com/huutvpn/permission/main/ip | grep $MYIP | awk '{print $3}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/huutvpn/permission/main/ip | awk '{print $4}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
echo "IZIN DI TERIMA!!"
else
...
[Unit]
Description=udp-custom by ©BADBUDSTORE
...
sleep 2
clear
curl "${repo}/Insshws/insshws.sh" | bash

#exp
cd /usr/bin
wget -O xp "${repo}/menu/xp.sh"
chmod +x xp
sleep 1
wget -q -O /usr/bin/notramcpu "${repo}/Finaleuy/notramcpu" && chmod +x /usr/bin/notramcpu
...
curl "${repo}/menu/insray.sh" | bash
sleep 1

curl "${repo}/arca.sh" | bash
sleep 1
#install slowdns
echo -e "\e[33m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
wget -q -O slowdns.sh ${repo}/SLDNS/slowdns.sh && chmod +x slowdns.sh && ./slowdns.sh
...
echo "------------------------------------------------------------"
echo ""
echo "===============-[ Autoscript Premium By @badbudstore ]-==============="
echo -e ""
echo ""
echo "" | tee -a log-install.txt
echo "ADIOS"
